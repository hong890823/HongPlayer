...
public class HRender implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener{
    public static int RENDER_MEDIA_CODEC = 1;
    public static int RENDER_YUV = 0;
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        LogUtil.logD("onSurfaceChanged, width:" + width + ",height :" + height);
        sWidth = width;
        sHeight = height;
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if(codecType == RENDER_MEDIA_CODEC)
        {
            renderMediacodec();
            LogUtil.logD("mediaocdec.......");
        }
        else if(codecType == RENDER_YUV)
        {
            renderYuv();
            LogUtil.logD("yuv.......");
        }
        ...
        if(cutimg)
        {
            ...
            {
                onGlSurfaceViewCreateListener.onCutVideoImg(bitmap);
            }
        }
    ...
    {
        String vertexShader = HShaderUtil.readRawTextFile(context, R.raw.vertex_base);
        String fragmentShader = HShaderUtil.readRawTextFile(context, R.raw.fragment_mediacodec);
        programId_mediacodec = HShaderUtil.createProgram(vertexShader, fragmentShader);
        ...
        HShaderUtil.checkGlError("glBindTexture mTextureID");
    ...
    {
        String vertexShader = HShaderUtil.readRawTextFile(context, R.raw.vertex_base);
        String fragmentShader = HShaderUtil.readRawTextFile(context, R.raw.fragment_yuv);
        programId_yuv = HShaderUtil.createProgram(vertexShader, fragmentShader);
    ...
    {
        String vertexShader = HShaderUtil.readRawTextFile(context, R.raw.vertex_base);
        String fragmentShader = HShaderUtil.readRawTextFile(context, R.raw.fragment_no);
        programId_stop = HShaderUtil.createProgram(vertexShader, fragmentShader);
